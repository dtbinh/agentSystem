...
subplot(1, 3, 1), imshow(sc_img), title('Server for each agent', 'FontSize', 16);
subplot(1, 3, 2), imshow(cap_img), title('Capacity for each agent', 'FontSize', 16);
subplot(1, 3, 3), imshow(p_img), title('Switching Probability for each agent', 'FontSize', 16);

%% Write Images to a Video
% Server video
rstVideo = VideoWriter('./rsts/agent_server.mp4', 'MPEG-4');
...
while ((sum(agent_alive > 0) < 800) || (sum(non_stable_agent) ~= 0)) && (iter < 500)
...
figure(2), title('The CDF of agent capacity');
cdfplot(agent_cap);
xlabel('The Capacity in agents');
ylabel('The cumulative probability');
